<template>
    <div class="relative">
        ...
        <button tabindex="-1" @click="visible = !visible">
            <svg class="w-4 h-4 hover:animate-spin " viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path clip-rule="evenodd" class="dark:fill-white"
        ...
        <!-- options -->
        <div v-if="visible"
            class="absolute top-6 w-48 z-10 flex flex-col gap-3 ring-1 bg-white dark:bg-gray-900 px-2 py-2 rounded shadow-xl">
            
            <div class="flex justify-between items-center">
                <span>Properties</span>
                <svg @click="visible=!visible" class="w-4 animate-bounce cursor-pointer h-4 fill-red-500" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
	...
	c-119.297,0-216-96.703-216-216S136.703,40,256,40s216,96.703,216,216S375.297,472,256,472z" />
            ...
            <div class="flex gap-2 items-center justify-between">
                <span>Color</span>
                <input :value="element.fill()" @input="$event => element.fill($event.target.value)" type="color" />
            </div>
            <div class="flex gap-2 items-center justify-between">
                <span>Font size</span>
                <input @input="$event => element.fontSize($event.target.value)" :value="element.fontSize()"
                    class="w-20 p-1 dark:bg-gray-800 rounded" type="number" />
            </div>

            <GeneralFieldConfig :element="element" />
...
<script>
import GeneralFieldConfig from './GeneralFieldConfig.vue';
export default {
    props: ['element'],
    components:{GeneralFieldConfig},
    data() {
        return {
            visible: false
        }
